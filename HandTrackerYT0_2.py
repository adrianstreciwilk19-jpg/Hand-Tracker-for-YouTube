...
def palec_w_gorze(dlon, tip_idx, pip_idx):
    return dlon[tip_idx].y < dlon[pip_idx].y

def kciuk_wystawiony(dlon):
    return abs(dlon[4].x - dlon[3].x) > 0.04
...
def trzy_palce(dlon):
    ...
    # serdeczny:  tip 16, pip 14
    # mały:       tip 20, pip 18
    wskazujacy = palec_w_gorze(dlon, 8, 6)
    srodkowy = palec_w_gorze(dlon, 12, 10)
    serdeczny = palec_w_gorze(dlon, 16, 14)
    maly = palec_w_gorze(dlon, 20, 18)
    kciuk = kciuk_wystawiony(dlon)

    return kciuk and wskazujacy and srodkowy and not serdeczny and not maly
    ...
    if wynik.hand_landmarks:
        ...
        if len(pozycje_x) >= 6:
            ...
            if waiting_for_reset:
                ...
                if trzy_palce(dlon) and dx > SWIPE_THRESHOLD and (teraz - last_action_time) > ACTION_COOLDOWN:
                    pyautogui.press('l')
                    print(">>>>")
                ...
                elif trzy_palce(dlon) and dx < -SWIPE_THRESHOLD and (teraz - last_action_time) > ACTION_COOLDOWN:
                    pyautogui.press('j')
                    print("<<<<")
                    last_action_time = teraz
                    block_until = teraz + RESET_BLOCK_TIME
